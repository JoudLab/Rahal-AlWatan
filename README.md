...
- Sound effects (correct, wrong, transition, arrival, victory) with a mute button; the choice is remembered
...
Five short clips live in `assets/audio/`:

| File | Played when |
|------|-------------|
| `correct.mp3` | a correct answer is chosen |
| `wrong.mp3` | a wrong answer is chosen |
| `transition.mp3` | "تابع الرحلة" is pressed |
| `station.mp3` | the robot arrives at a new city |
| `victory.mp3` | the journey is completed |

- Browsers block audio until the user interacts with the page, so sound starts after the first tap or click.
- The speaker button (top-left, on every screen) mutes and unmutes the effects. The choice is saved in `localStorage`.
- To replace a sound, drop a new file with the same name into `assets/audio/`. A missing file is skipped silently.
- The bundled clips were synthesized for this project (no third-party audio).
...
- Web Audio API for sound effects, with an HTMLAudio fallback
...
**Option 1:** open `index.html` in a browser (sound also works this way, through a fallback player).
...
│   └── sound.js      # Sound effects and mute button
├── assets/
│   ├── robot.webp    # Robot mascot
│   └── audio/        # correct, wrong, transition, station, victory (.mp3)
